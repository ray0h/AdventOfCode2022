...
# myfile = open("/Users/raymondoh/Documents/AdventOfCode2022/Day7/test.txt", "r").readlines()

myfile = open("/Users/raymondoh/Documents/AdventOfCode2022/data/day7.txt", "r").readlines()
...
def is_top_level(string: str) -> bool:
    return string == "$ cd /"

def is_a_directory(string: str) -> bool:
    return 'dir ' in string

def is_a_file(string: str) -> bool:
    regex = '\d+'
    return len(re.findall(regex, string)) != 0

def enter_directory(string: str) -> bool:
    if 'cd ' not in string:
        return False
    else:
        return string.split(' ')[2] != '..'

def exit_directory(string: str) -> bool:
    if 'cd ' not in string:
        return False
    else:
        return string.split(' ')[2] == '..'
    
def create_file(string: str, current: Folder) -> None:
    size, name = string.split(' ')
    current.files.append({'name': name, 'size': int(size)})

def create_directory(string: str, current: Folder) -> None:
    _label, name = string.split(' ')
    current.subfolders.append(Folder(name=name, parent=current))

coms = [line.rstrip() for line in myfile]
...
for line in coms:
    if is_top_level(line):
        current = root
    elif is_a_directory(line):
        dir_name = line.split(' ')[1]
        build_list.append(dir_name)
        create_directory(string=line, current=current)
    elif is_a_file(line):
        create_file(string=line, current=current)
    elif enter_directory(line):
        dir_name = line.split(' ')[2]
        sub = list(filter(lambda dir: dir.name == dir_name, current.subfolders))[0]
        current = sub
    elif exit_directory(line):
...
# determine directory sizes (inclusive)
queue = []
...
while len(queue) > 0:
    ...
    # calculate total size of files in current folder
    filesum = 0
    for file in temp.files:
        filesum += file['size']
    
    # add subfolders to queue
    for subdir in temp.subfolders:
        queue.append(subdir)
    
    # add to files size to Folder object
    temp.size += filesum
    
    # see if current folder should be added to parent folder file size sums:
    while temp.parent_folder is not None:
        temp.parent_folder.size += filesum
        temp = temp.parent_folder


# traverse and see what directories are <= 100000 size
# reset current to root
current = root
...
while len(queue) > 0:
    ...
    # check if <= 100000 size
    if temp.size <= 100000:
        dir_sizes.append({'name': temp.name, 'value':temp.size})
    
    # add subfolders to queue
    for subdir in temp.subfolders:
        queue.append(subdir)

hundred_k_or_less = [d['value'] for d in dir_sizes]

hundred_k_or_less_sum = sum(hundred_k_or_less)
print(f"{hundred_k_or_less_sum = }")
...
additional_space_needed = NEEDED_SPACE - (TOTAL_SPACE - root.size)

# traverse and see what directories are <= additional_space size
...
while len(queue) > 0:
    ...
    # check if >= additional_space needed size
    if temp.size >= additional_space_needed:
        dir_sizes.append({'name': temp.name, 'value':temp.size})
...
large_enough_dirs = [d['value'] for d in dir_sizes]
smallest_enough_to_delete = min(large_enough_dirs)

print(f"{smallest_enough_to_delete = }")