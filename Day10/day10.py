...
# myfile = open("/Users/raymondoh/Documents/AdventOfCode2022/Day10/test.txt", "r").readlines()
# myfile = open("/Users/raymondoh/Documents/AdventOfCode2022/Day10/test2.txt", "r").readlines()

myfile = open("/Users/raymondoh/Documents/AdventOfCode2022/data/day10.txt", "r").readlines()
...
def parse_line(string: str):
    ...
    if string == 'noop':
        ...
        instruction, value = string.split(' ')
...
for line in myfile:
    ...
    if instruction == 'noop':
        ...
        cycles.append(last_cycle)
        cycles.append(last_cycle + value)

# print(f"{cycles = }")
cycle_list = [20, 60, 100, 140, 180, 220]

signal_strength = 0
for val in cycle_list:
    print(f"{val = }, {cycles[val-1] = }, {val * cycles[val-1] = }")
    signal_strength += val * cycles[val-1]
...
# (In this system, there is no such thing as "vertical position": if the sprite's horizontal position puts its pixels where 
...
for row in range(0, 6):
    grid.append('')
    for relative_cycle in range(1, 41):
        current_cycle = row * 40 + relative_cycle
        current_sprite = cycles[current_cycle-1]
        # print(f"{row = }, {relative_cycle = }, {current_cycle = }, {cycles[current_cycle - 1] = }, {current_sprite = }")

        sprite_range = [current_sprite - 1, current_sprite, current_sprite + 1]
        if relative_cycle-1 in sprite_range:
            grid[row] += '#'
        else:
            grid[row] += '.'
