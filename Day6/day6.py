...
file = open("/Users/raymondoh/Documents/AdventOfCode2022/data/day6.txt", "r").readlines()
buffer = file[0]

UNIQUE_MARKER_LENGTH = 4

def is_packet_unique(ind: int, buffer: str) -> bool:
    packet = set(list(buffer)[ind : ind + UNIQUE_MARKER_LENGTH])
    return len(packet) == UNIQUE_MARKER_LENGTH

value = None
for ind in range(0, len(buffer) - UNIQUE_MARKER_LENGTH):
    if is_packet_unique(ind, buffer):
        value = ind + UNIQUE_MARKER_LENGTH
        break
