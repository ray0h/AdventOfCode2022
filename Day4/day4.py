...
file = open("/Users/raymondoh/Documents/AdventOfCode2022/data/day4.txt", "r").readlines()

from typing import List
import re

def parse_bounds(line: str) -> List[str]:
    digits_regex = "\d+" 
    return re.findall(digits_regex, line)

def is_contained_ranges(bounds: List[str]) -> bool:
    start_one = int(bounds[0])
    end_one = int(bounds[1])
    start_two = int(bounds[2])
    end_two = int(bounds[3])

    return (start_one <= start_two and end_one >= end_two) or (start_two <= start_one and end_two >= end_one)

assignment_bounds = [parse_bounds(el) for el in file]
contained_ranges = [is_contained_ranges(bounds) for bounds in assignment_bounds]

print(f"No of pairs where one range contains the other: {contained_ranges.count(True)}")
...
def is_overlapping_ranges(bounds: List[str]) -> bool:
    start_one = int(bounds[0])
    end_one = int(bounds[1])
    start_two = int(bounds[2])
    end_two = int(bounds[3])

    range_one = range(start_one, end_one + 1)
    range_two = range(start_two, end_two + 1)

    return True if set(range_one) & set(range_two) else False

overlapping_ranges = [is_overlapping_ranges(bounds) for bounds in assignment_bounds]

print(f"No of pairs where one range overlaps the other: {overlapping_ranges.count(True)}")