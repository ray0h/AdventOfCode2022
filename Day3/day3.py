...
file = open("/Users/raymondoh/Documents/AdventOfCode2022/data/day3.txt", "r").readlines()
...
def find_common_value(str_list):
    setlist = [set(el.rstrip()) for el in str_list]
    common_value = set.intersection(*setlist)
    return list(common_value)[0]

def calculate_priority(ch):
    # ord('A') = 65
    # ord('a') = 97
    offset = 96 if ch.islower() else 38
    
    return ord(ch) - offset

pairs_list = [split_string(el) for el in file]
common_items = [find_common_value(el) for el in pairs_list]
priority_values = [calculate_priority(el) for el in common_items]

import functools
import operator

priority_sum = functools.reduce(operator.add, priority_values)
...
for i in range(0, len(file)-2, 3):
    group_badge = find_common_value([file[i], file[i+1], file[i+2]])
    badge_list.append(group_badge)

part_two_priority_values = [calculate_priority(el) for el in badge_list]
part_two_priority_sum = functools.reduce(operator.add, part_two_priority_values)
print(f"{part_two_priority_sum = }")